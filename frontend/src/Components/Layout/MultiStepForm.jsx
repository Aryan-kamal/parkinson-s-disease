...
const steps = [
	...
	{
		title: "Step 1",
		fields: ["username", "mdvpFo", "mdvpFhi", "mdvpFlo", "mdvpJitterPercent"],
	},
	{
		title: "Step 2",
		fields: [
...
const MultiStepForm = () => {
	...
	const onSubmitForm = async () => {
		try {
			setLoading(true);
			console.log("Backend URL:", process.env.REACT_APP_BACKEND_FASTAPI_URL);
			const {data} = await axios.post(
				`${process.env.REACT_APP_BACKEND_FASTAPI_URL}`,
				{
			...
			if (data.pred_value === 0) {
				...
				dispatch(
					setResult(
						`Dear ${formData.username}, you don't have Parkinson Disease.`
					)
				);
				...
				dispatch(
					setResult(`Dear ${formData.username}, you have Parkinson Disease.`)
				);
		...
		<div>
			...
			<div className="arrow-container">
				<button
					onClick={prevStep}
					disabled={step === 0}
					className={step === 0 ? "disabled-button" : ""}
				>
					<BsFillArrowLeftCircleFill className="arrow" />
				</button>
				{step === steps.length - 1 ? (
					<button
						className={isAllFieldsEmpty() ? "disabled-button" : "btn"}
						type="button"
